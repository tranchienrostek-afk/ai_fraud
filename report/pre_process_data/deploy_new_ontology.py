...
def migrate():
    ...
    with driver.session(database=DB) as session:
        ...
        hosp_map = dict(zip(hospitals_df['hospital_code'], hospitals_df['hospital_name']))
        ...
        // Create Doctors (Exam and Admit)
        WITH cl, row
        FOREACH (doc_name IN CASE WHEN row.doctor_name_exam IS NOT NULL THEN [row.doctor_name_exam] ELSE [] END |
            MERGE (dr:Doctor {doctor_name: doc_name})
            MERGE (cl)-[:EXAMINED_BY {role: 'exam'}]->(dr)
        )
        FOREACH (doc_name IN CASE WHEN row.doctor_name_admit IS NOT NULL THEN [row.doctor_name_admit] ELSE [] END |
            MERGE (dr:Doctor {doctor_name: doc_name})
            MERGE (cl)-[:EXAMINED_BY {role: 'admit'}]->(dr)
        )
        """
        ...
        WITH e, row.claim_id AS claim_id, row.drug_or_service_name AS item_name
        MATCH (cl:Claim {claim_id: claim_id})
        MERGE (cl)-[:HAS_EXPENSE]->(e)
        
        WITH e, item_name
        WHERE item_name IS NOT NULL
        MERGE (ds:DrugOrService {name: item_name})
        MERGE (e)-[:IS_ITEM]->(ds)
        """
        ...
        benefit_query = """
        UNWIND $batch AS row
        // Composite key: claim_id + benefit_name to avoid duplication within the same claim
        ...
        WITH bt, row.claim_id AS claim_id
        MATCH (cl:Claim {claim_id: claim_id})
